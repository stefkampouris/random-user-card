...
const UserCardServer: React.FC = () => {
  const [user, setUser] = useState<User | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [isExpanded, setIsExpanded] = useState<boolean>(false);

  // Function to fetch user data from our Flask backend
  const fetchUser = async (gender?: string) => {
    setLoading(true);
    try {
      const url = gender 
        ? `http://localhost:5000/user?gender=${gender}`
  ...
  // Add these handler functions after the GenderIcon component
  ...
  return (
    <div className={`flex flex-col items-center w-full p-3 ${inter.className}`}>
      ...
      <div className="w-full max-w-[22rem] lg:max-w-[26rem] xl:max-w-[30rem] mt-4 flex flex-col sm:flex-row gap-2">
        <Button 
          onClick={() => fetchUser('male')} 
          variant="outline" 
          className="sm:flex-1 text-base py-3 gap-2 border-[#f27c22] text-[#f27c22] hover:bg-[#f27c22] hover:text-white"
          disabled={loading}
        >
          {loading ? (
            <>
            ...
            <>
              <FontAwesomeIcon icon={faMars} className="w-5 h-5 text-blue-500" />
              Male
            </>
          )}
        </Button>
        <Button 
          onClick={() => fetchUser('female')} 
          variant="outline" 
          className="sm:flex-1 text-base py-3 gap-2 border-[#f27c22] text-[#f27c22] hover:bg-[#f27c22] hover:text-white"
          disabled={loading}
        >
          {loading ? (
            ...
            <>
              <FontAwesomeIcon icon={faVenus} className="w-5 h-5 text-pink-500" />
              Female
            </>
          )}
        </Button>
